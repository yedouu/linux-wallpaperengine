...
方案 v2（2026-08-06）：完成以上代码后，先冻结功能范围并执行单显示器测试。本机已实现 systemd 用户服务与优雅退出（`main.cpp` 同时注册 SIGINT/SIGTERM，`lwe stop` 即发送 SIGTERM），因此**退出方式统一为 SIGTERM，不再使用 Ctrl+C**。

环境：单显示器 2560x1440，GNOME Shell 42.9 + X11，`--gnome-x11` 桌面模式。原则：只验收、不加新功能。

#### A. 启动与退出生命周期

- A1 前台启动 `--gnome-x11`，与默认窗口模式对照，两种模式互不干扰。
- A2 systemd 服务启动（`lwe install → enable → start`），`lwe status` 为 active。
- A3 SIGTERM 退出：服务方式 `lwe stop`，前台方式 `kill -TERM <pid>`；预期程序输出 `Stop requested by signal`，GNOME 原背景自然露出。
- A4 退出后无残留：`wmctrl -l` / htop 查窗口与进程，无孤儿进程、无残留窗口。
- A5 退出后立即重启：单实例锁（flock）随进程退出由内核释放，可直接再起。

#### B. 窗口层级

- B1 打开普通应用（终端、Clash、文件管理器）→ 壁纸恒在应用之下。
- B2 最小化 → 最大化 → 还原，全程层级正确、无遮挡。
- B3 Alt+Tab 切换：壁纸不在切换列表；切换动画短暂露背景为 GNOME compositor 限制（见 4.8 节），记录备注但不阻塞验收。

#### C. 工作区

- C1 新建 4 个工作区循环切换 → 壁纸在所有工作区可见（`STICKY` 生效）。
- C2 切换瞬间无黑屏、无闪烁、无抖动。

#### D. Activities Overview

- D1 打开活动总览 → 壁纸正常、不抢焦点、不卡顿。
- D2 关闭总览 → 完整恢复。

#### E. 桌面交互（点击穿透）

- E1 桌面图标可见、可点击、可拖拽。
- E2 桌面右键菜单正常弹出（验证 XShape 空输入区域）。

#### F. 锁屏/解锁

- F1 锁屏：壁纸窗口被锁屏界面覆盖，为 GNOME 预期行为（锁屏由 shell 独立合成，非缺陷；要锁屏显示动态壁纸需 Shell 扩展，见 4.8 节同类限制）。
- F2 解锁：壁纸完整恢复、首帧无条纹。
- F3 锁屏期间全屏暂停生效，CPU 占用下降。

#### G. 内容类型

- G1 Scene 壁纸（如 2955458015）长跑。
- G2 Video 壁纸（mpv 循环、音量）。
- G3 Web 壁纸跳过（CEF 已知问题，低优先级，见 11.6.3）。

#### H. 稳定性

- H1 `--cycle` 循环切换 30 分钟以上，无崩溃、无卡死、无内存泄漏。
- H2 进程退出后无残留实例（htop 检查）。

#### 验收标准

- 层级始终正确：GNOME 背景 < 壁纸 < 普通应用。
- 不抢焦点、不出现在任务栏/Alt+Tab。
- 不遮挡桌面交互。
- SIGTERM 退出后背景自然露出、无残留窗口/进程。
- 长跑无崩溃、无卡死。

#### 与后续步骤的衔接

- 每个用例记录 Pass/Fail 到本文档，全部通过后再进步骤 6（单窗口多显示器 viewport）。
- 若 F2 出现条纹，回退排查 4.3 节 framebuffer 清理路径。
- 若 H1 崩溃，回溯 11.6 节已修复的悬垂监听器 / GL 函数指针路径。
- 锁屏时显示动态壁纸需 GNOME Shell 扩展（与 4.8 节同类），不阻塞本步骤验收。

#### 实测记录（2026-08-06）

在真实 GNOME 42.9 X11 桌面（2560x1440，`--gnome-x11 --cycle` 服务）实测：

| 用例 | 结果 | 备注 |
|------|:---:|------|
| A1-A5 启动/退出/重启 | ✅ 全过 | SIGTERM 优雅退出（日志含 `Stop requested by signal`）、无残留、flock 释放后可立即重启 |
| B1-B3 层级 | ✅ 通过 | 堆叠最底 + DOCK/BELOW/STICKY/SKIP_TASKBAR/SKIP_PAGER；`input=False` 不可聚焦；最大化/最小化后层级不变 |
| C1 所有工作区 | ✅ 通过 | `_NET_WM_DESKTOP=0xFFFFFFFF` |
| C2 切换无闪烁 | ✅ 通过 | 模拟 Ctrl+Alt+Down/Up 切换，壁纸窗口保持 `IsViewable`、渲染非黑屏 |
| D Overview | ✅ 通过 | Super 开/关 Overview，壁纸保持可见、渲染正常 |
| E1/E2 点击穿透 | ✅ 通过 | ShapeInput 空矩形（0 个，XShapeGetRectangles 实测），点击直达桌面 |
| F1 锁屏覆盖 | ✅ 预期 | 锁屏激活（GetActive=true）时壁纸窗口位于锁屏层下 |
| F2 解锁恢复 | ✅ 通过 | 解锁后壁纸 `IsViewable`、渲染非黑屏、无条纹 |
| F3 锁屏暂停 | ⚠️ 未达预期 | 锁屏期间 CPU 未降（10.5% vs 10.6%）：全屏暂停检测不到锁屏（锁屏是 compositor 层而非全屏窗口），建议后续加锁屏状态检测 |
...
1. **步骤 5**：单显示器桌面验收（方案 v2：SIGTERM 退出 + 锁屏只验解锁恢复，详见 8.6 节）。
